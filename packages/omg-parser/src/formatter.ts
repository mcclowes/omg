 ...
 */
export function formatDocument(content: string, options: FormatOptions = {}): string {
 ...
 */
function formatFrontmatter(data: Record<string, unknown>, sort: boolean, order: string[]): string {
 ...
 */
function formatYamlValue(key: string, value: unknown, depth: number): string {
  ...
  if (Array.isArray(value)) {
    ...
    const items = value.map((v) => {
      if (typeof v === 'object' && v !== null) {
        const objLines = Object.entries(v as Record<string, unknown>).map(([k, val]) =>
          formatYamlValue(k, val, depth + 2)
        );
        return `${indent}  -\n${objLines.join('\n')}`;
      }
      return `${indent}  - ${v}`;
  ...
  if (typeof value === 'object') {
    const objLines = Object.entries(value as Record<string, unknown>).map(([k, v]) =>
      formatYamlValue(k, v, depth + 1)
    );
    return `${indent}${key}:\n${objLines.join('\n')}`;
  }
 ...
 */
function formatBody(body: string, indent: number): string {
 ...
 */
function formatOmgSchema(content: string, indentSize: number): string {
 ...
 */
function formatSchemaContent(content: string, indentSize: number): string {
 ...
 * Format a JSON block
 */
function formatJsonBlock(content: string, indentSize: number): string {
  const trimmed = content.trim();
  if (!trimmed) return '';
