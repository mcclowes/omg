...
// Severity levels (matching Spectral)
...
interface SpectralConfig {
  ...
  functionsDir?: string;
  functions?: string[];
  rules: Record<string, RuleDefinition | Severity | boolean>;
}

/**
 * Load and parse the Spectral OMG config
 */
function loadConfig(configPath: string): SpectralConfig | null {
  if (!fs.existsSync(configPath)) {
    return null;
  }

  const content = fs.readFileSync(configPath, 'utf-8');
  return yaml.load(content) as SpectralConfig;
}
...
function evaluateCondition(
  ...
  if (condition.field && target && typeof target === 'object') {
    value = (target as Record<string, unknown>)[condition.field];
  }
  ...
  switch (condition.function) {
    ...
    case 'enumeration': {
      const options = condition.functionOptions as { values: unknown[] };
      return options?.values?.includes(value) ?? true;
    }

    case 'pattern': {
      const options = condition.functionOptions as {
        match?: string;
        notMatch?: string;
      };
    ...
    case 'length': {
      const options = condition.functionOptions as { min?: number; max?: number };
    ...
    case 'casing': {
      const options = condition.functionOptions as { type: string };
      if (typeof value !== 'string') return true;
...
function evaluateOmgResponseRequired(
  ...
  if (options?.requireSuccess) {
    const hasSuccess = responseBlocks.some((b) => {
      if (b.type === 'omg.response') return true;
...
function evaluateOmgAnnotationValid(
  ...
  switch (rule) {
    case 'get-no-body': {
      const method = (doc?.frontMatter as Record<string, unknown>)?.method;
      const blocks = (doc?.resolvedBlocks || doc?.blocks || []) as Array<{
        type: string;
      }>;
    ...
    case 'list-needs-pagination': {
      ...
      const blocks = (doc?.resolvedBlocks || doc?.blocks || []) as Array<{
        type: string;
        parsed?: { kind: string; properties?: Record<string, unknown> };
      }>;
      const queryBlock = blocks.find((b) => b.type === 'omg.query');
      if (!queryBlock?.parsed) return false;
...
function evaluateOmgPathParameter(
  ...
  while ((match = paramRegex.exec(target)) !== null) {
    ...
    if (casing === 'camelCase') {
      if (!/^[a-z][a-zA-Z0-9]*$/.test(param)) return false;
    }
  }
 ...
 */
export function evaluateOalPropertyCasing(
  target: unknown,
  options?: Record<string, unknown>
): boolean {
  const doc = target as Record<string, unknown>;
  ...
  const expectedCasing = (options?.casing as string) || 'camel';
 ...
 */
function findInvalidCasingProperties(
  schema: unknown,
 ...
 */
export function checkCasing(name: string, casing: CasingType | string): boolean {
  switch (casing) {
    case 'camel':
 ...
 */
function formatMessage(template: string, value: unknown): string {
