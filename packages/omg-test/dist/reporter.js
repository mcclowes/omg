 ...
 */
export function formatResults(summary, format, verbose = false) {
    switch (format) {
        case 'json':
 ...
 */
function formatConsole(summary, verbose) {
    const lines = [];
    // ANSI color codes
    const green = '\x1b[32m';
    const red = '\x1b[31m';
 ...
 */
function formatJunit(summary) {
    const lines = [];
    lines.push('<?xml version="1.0" encoding="UTF-8"?>');
    lines.push(`<testsuite name="Contract Tests" tests="${summary.total}" failures="${summary.failed}" skipped="${summary.skipped}" time="${(summary.duration / 1000).toFixed(3)}">`);
    for (const result of summary.results) {
