 ...
 */
export async function runContractTests(inputPath, options) {
    ...
    const allEndpoints = extractEndpoints(openapi);
...
function filterEndpoints(endpoints, filter) {
    ...
    return endpoints.filter((endpoint) => {
        for (const f of filter) {
            ...
            if (endpoint.operationId === f)
                return true;
            // Match by path
            if (endpoint.path === f)
                return true;
            // Match by method + path (e.g., "GET /health")
            if (`${endpoint.method} ${endpoint.path}` === f.toUpperCase())
                return true;
            // Match by glob pattern on path
            if (f.includes('*') && matchGlob(endpoint.path, f))
                return true;
        }
        return false;
    });
