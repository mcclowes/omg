...
describe('importOpenApi', () => {
    ...
    it('includes baseUrl from servers', () => {
        ...
        expect(result.api.frontMatter.baseUrl).toBe('https://api.example.com/v1');
    });
    it('includes contact info', () => {
        ...
        const result = importOpenApi(spec);
        expect(result.api.frontMatter.contact).toEqual({
            name: 'API Support',
            email: 'support@example.com',
            url: 'https://example.com/support',
        });
    ...
    describe('endpoint conversion', () => {
        it('converts GET endpoint', () => {
            ...
            expect(endpoint.frontMatter.method).toBe('GET');
            expect(endpoint.frontMatter.path).toBe('/users');
            expect(endpoint.frontMatter.operationId).toBe('list-users');
            expect(endpoint.frontMatter.tags).toEqual(['Users']);
        });
        it('converts multiple methods on same path', () => {
            ...
            const methods = result.endpoints.map((e) => e.frontMatter.method);
        ...
        it('generates operationId if not provided', () => {
            ...
            const result = importOpenApi(spec);
            expect(result.endpoints[0].frontMatter.operationId).toBe('get-users-by-id');
        });
        it('converts path parameters', () => {
            ...
            expect((pathBlock?.parsed).properties.id.type).toBe('uuid');
        });
        it('converts query parameters', () => {
            ...
            expect((queryBlock?.parsed).properties.limit).toBeDefined();
            expect((queryBlock?.parsed).properties.offset).toBeDefined();
        });
        it('converts request body', () => {
            ...
            expect((bodyBlock?.parsed).properties.name).toBeDefined();
        });
        ...
        it('handles deprecated flag', () => {
            ...
            const result = importOpenApi(spec);
            expect(result.endpoints[0].frontMatter.deprecated).toBe(true);
        });
        it('handles x-follows extension', () => {
            ...
            const result = importOpenApi(spec);
            expect(result.endpoints[0].frontMatter.follows).toEqual([
                'create-cart',
                'add-items',
            ]);
        });
